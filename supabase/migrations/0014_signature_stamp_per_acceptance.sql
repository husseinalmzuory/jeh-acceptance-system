-- Per-acceptance control for showing the editor signature and journal stamp.

alter table public.acceptances
  add column if not exists include_signature_stamp boolean not null default true;

-- Existing acceptances keep the historical behavior: signature + stamp shown.
update public.acceptances
set include_signature_stamp = true
where include_signature_stamp is null;

-- Replace create RPC with a version that persists the fourth required option.
...
create or replace function public.create_acceptance(
  ...
  p_include_signature_stamp boolean
)
returns uuid
language plpgsql
security definer
...
begin
  ...
  if p_is_extracted_research is null
     ...
     or p_is_non_arabic_language is null
     or p_include_signature_stamp is null then
    raise exception 'All acceptance options are required';
  end if;
  ...
  insert into public.acceptance_versions (
    ...
    jsonb_build_object(
      ...
      'include_signature_stamp', p_include_signature_stamp,
...
revoke all on function public.create_acceptance(
  text, text, text, date, date, date, date, text, jsonb, boolean, boolean, boolean, boolean
) from public;
...
-- Replace update RPC so edits persist and version this option too.
...
revoke all on function public.update_acceptance(
  uuid, text, text, text, date, date, date, date, text, jsonb, boolean, boolean, boolean, boolean, text
) from public;
